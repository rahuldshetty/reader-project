<script lang="ts">
    ...
    selected_post.subscribe(async (curValue)=>{
        if(curValue){
            ...
            Parser.parse(curValue.link, {
                html: webpage_content
            }).then(result => {
...
{:else}
    ...
    {#if parsed.lead_image_url}
        <img src={parsed.lead_image_url} alt={parsed.title} class="rounded-md object-cover">
    {/if}

    <div class="mt-4 mr-4 indent-0 text-text1 text-base font-normal leading-relaxed text-justify mb-20">
        <article>
            {@html parsed.content}
        </article>
    </div>
